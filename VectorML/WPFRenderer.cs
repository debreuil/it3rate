...
{
	public class WPFRenderer : XamlBaseRenderer
	{
		public override void GenerateML(VexObject v, string path, out string xamlFileName)
        {
		...
		public override void GenerateMLPart(VexObject v, IDefinition def, out string xamlFileName)
		{
			this.v = v;
			this.Log = new StringBuilder();
		...
		public void WriteStoryboards(List<IInstance> instances, bool isRoot)
		{
		  ...
		  //</Canvas.Triggers>
			...
			for (int i = 0; i < instances.Count; i++)
			{
                ...
                instances[i].InstanceHash = (uint)i;
		...
		public override void WriteInstance(IDefinition s, Instance inst)
		{
            ...
            if (!INCLUDE_ANIMATION)
            {
				...
				WriteTransformDef(inst, fullName);
            }

            string prefix = (s is Image) ? imageBrushPrefix : visualBrushPrefix;

			xw.WriteStartElement("Rectangle");
			xw.WriteStartAttribute("x:Name");
			xw.WriteValue(instancePrefix + prefix + instName);
			xw.WriteEndAttribute();

			xw.WriteStartAttribute("Fill");
            xw.WriteString("{StaticResource " + prefix + inst.DefinitionId + "}");
			xw.WriteEndAttribute();
